      ...
      <View style={styles.grid}>
        {folders.map((f) => (
          ...
          >
            ...
            <Text style={styles.folderSub}>{f.source === 'official' ? '運営提供' : 'マイフォルダ'}</Text>
          </Pressable>
        ))}
        <Pressable style={[styles.folderCard, styles.addCard]} onPress={() => setShowAddFolder(true)}>
          <Ionicons name="add-circle-outline" size={22} color={colors.textSecondary} />
          <Text style={styles.folderTitle}>新規カテゴリ</Text>
        </Pressable>
      </View>
